...
def model_maker(input_shape, num_classes):
    ...
    x = keras.layers.Activation("relu")(x)
    xs1 =keras.layers.Conv2D(32,1, strides=2)(x) #skep Connection

    # Block 2
    px = keras.layers.MaxPooling2D((3,3),1,"same")(x)
    px = keras.layers.Conv2D(32, 3, padding="same")(px)
    px = keras.layers.Activation("relu")(px)
    px = keras.layers.Conv2D(36, 3, padding="same")(px)
    xs2 =keras.layers.Conv2D(48,1, strides=2)(px) #skep Connection 2

    # block3-1
    px = keras.layers.add([px,xs1])
    px = keras.layers.Activation("relu")(px)
    ...
    # flatten
    ex = keras.layers.Conv2D(256, 3, padding="same")(px)
